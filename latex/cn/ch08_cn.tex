...
教材视角下，宜把 LLM 看作强大的条件文本生成器：其知识（1）在参数记忆中冻结于训练截止期；（2）又极易被提示上下文改写。正因这种双重性，当答案必须引用真实 Asset-ID 时，接地策略比模型品牌更重要。面向建筑应用的领域导读同样强调机遇与可靠性边界并存~\cite{ma2026tenquestions,saka2024gptconstruction}。
...
\end{tikzpicture}
\caption{巡检对话中 LLM 的能力阶梯。流畅与分步表述先于可审计性；接地是单独的工程层。}
...
\end{tikzpicture}
\caption{感知、孪生/GIS 与语言栈的角色分工。VLM 可协助描述，但不得虚构 Asset-ID。}
\label{fig:ch8-vlm-roles}
\end{figure}

\subsection{失效模式：幻觉、错误编号与不可审计建议}
幻觉综述表明：流畅的自然语言生成系统在证据缺失或检索薄弱时，很容易虚构实体、数字与引用~\cite{ji2023}。对立面运维而言，高代价失效非常具体：虚构 Asset-ID；缺乏可归因条款的维修建议；流畅却默默丢掉 RGB/IR 矛盾证据的摘要。可解释性综述也强调：事后口头辩解≠证据接地~\cite{zhao2024}。面向建筑的 GPT 类工具综述同样警告：机遇很大，但领域可靠性需要外部知识与流程控制~\cite{saka2024gptconstruction,ma2026tenquestions}。
...
\end{tikzpicture}
\caption{四种记忆体制。对立面项目，孪生才是长期权威；LLM 应通过检索与工具读取它。}
...
\end{tikzpicture}
\caption{本章一以贯之的接地栈。每一层消解不同失效模式；Asset-ID 结构是 FM 行动前的最后闸门。}
...
相对纯参数化 GPT，RAG 能改善建筑规范与安全手册问答~\cite{joffe2025buildingcodes,lee2024ragsafety}；混合检索也被用于敏感的缺陷管理对话~\cite{jeon2025hybridrag}；设施文档 RAG 则展示对运维手册与图纸的安全检索~\cite{krutli2025fmrags}。图导向检索综述与施工计划应用表明，结构化语料有助于打破信息孤岛~\cite{peng2024}。对第8章的启示：带版本的专家语料很有价值，但必须与网页抓取以及孪生观测库分离。

\paragraph{能耗、运维与面向机器人的智能体.}
能耗建模的 agent schema~\cite{zhang2025llmagentschema}，用户—孪生—机器人之间的 LLM 经纪~\cite{luo2025llmdtrobot}，原生 IFC 上的模式引导多智能体工具组合~\cite{gao2026ifcagent}，带混合记忆的施工 LLM 多智能体框架~\cite{rawat2025llmmas}，以及 BIM 内孪生的多智能体虚拟建模~\cite{lee2025multiagentvbm}，共同标出从「能答」到「能行动」的轨迹。认知孪生讨论同样强调可互操作知识上的检索~\cite{shi2025cognitivedt}。
